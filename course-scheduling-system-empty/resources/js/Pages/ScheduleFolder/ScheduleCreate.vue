...
export default {
  ...
  computed: {
    semesterSchedule(){
      const arr = this.response.filter(teacher => this.form.content.includes(teacher.id));
      return arr;
    }
  },
  methods: {
    ...
    storeSchedule() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.$inertia.post(route('schedule.store'), fromData, {
            onSuccess: (res) => {
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">新增課表</h2>
    </template>
          ...
          <form class="flex flex-col gap-y-4" @submit.prevent="storeSchedule">
            ...
            <div class="flex items-center justify-between">
              <div class="w-[450px] h-[500px] border border-black rounded-md overflow-y-auto">
                ...
                <button v-for="teacher in response" :key="teacher.id" type="button" class="grid grid-cols-2 w-full p-2 border-b text-center cursor-pointer hover:bg-slate-50" @click="selectTeacher(teacher.id)" :class="{ 'selected' :selectedTeacher === teacher.id }">
