...
export default {
  ...
  data() {
    return {
      ...
      templatePhoto: this.response.photo,
    };
  },
  methods: {
    putImg(e) {
      ...
      storeCourse() {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.$inertia.post(route('course.update',{ id:this.response.id }), this.form, {
              onSuccess: (res) => {
        ...
        <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <form class="flex flex-col gap-y-4"  @submit.prevent="storeCourse">
            ...
            <div>
              ...
              <label for="photo" class="flex items-center justify-center w-[356px] h-[200px] mt-1 text-gray-400 bg-gray-100 rounded-md text-2xl font-bold cursor-pointer">
            ...
            <div class="flex justify-center gap-x-4 pt-8">
              <Link :href="route('course.index')" class="inline-block px-4 py-2 rounded-md bg-gray-500 text-white font-bold">
                返回列表
              </Link>
